...
const char * const BITCOIN_CONF_FILENAME = "neuralinkbrainkeycoin.conf";
const char * const BITCOIN_PID_FILENAME = "neuralinkbrainkeycoin.pid";
...
#else
    const char* pszModule = "neuralinkbrainkeycoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Neuralinkbrainkeycoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Neuralinkbrainkeycoin";
#else
    // Unix
    return pathRet / ".neuralinkbrainkeycoin";
#endif
#endif
}
